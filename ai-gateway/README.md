...
- **STT** — faster-whisper (polecenia głosowe) — etap 20,
- **RAG** — pamięć botów i wiedza z kompendium — etap 18.
...
Nagłówek `X-API-Key` jest wymagany, gdy ustawisz `GATEWAY_API_KEY` (obowiązkowe po etapie 27 — gateway będzie osiągalny z VPS przez Tailscale). `GET /health` celowo zostaje bez klucza: serwer VTT odpytuje go cyklicznie, żeby pokazywać status botów.
...
**Decyzja na etap 20 (STT):** domyślny kontekst **32 768** zostawia ~4,0 GB wolnego VRAM. faster-whisper `medium` w float16 potrzebuje ~1,5–2 GB, więc mieści się bez schodzenia na `small` ani na CPU. Gdyby zabrakło (np. po dołożeniu RAG), kolejność cięć: kontekst 16k → `--cache-type-k q8_0 --cache-type-v q8_0` → whisper `small`.
...
- **Model zmyśla zasady Cyberpunk RED** (np. „Sztuka Klatki (Parkour), DC 15–16” zamiast Atletyki i DV). Bez RAG na podręczniku to normalne — naprawia to etap 18. Do tego czasu asystent MG jest wyłącznie zabawką diagnostyczną.
