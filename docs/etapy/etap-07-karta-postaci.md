...
- [x] Model `Character` (JSON w kolumnie + kluczowe pola relacyjne): statystyki (INT, REF, DEX, TECH, COOL, WILL, LUCK, MOVE, BODY, EMP), HP bieżące/max (wyliczane z BODY+WILL), rola i ranga zdolności roli, humanity, lista umiejętności z poziomami, ekwipunek (prosta lista — kompendium dojdzie w etapie 13), eddies, notatki, portret
- [x] Definicje umiejętności CP RED (polskie nazwy, przypisana statystyka) jako dane w `data/public/` — struktura umożliwiająca podmianę na pełne dane z podręcznika w etapie 13
...
- Klikalne rzuty (etap 08), kompendium i przedmioty z danymi (etap 13), cyberware/humanity mechanika (etap 23), generator postaci (etap 25)
...
- Schemat postaci wersjonuj (`schemaVersion` w JSON) — ułatwi migracje, gdy w etapach 13/23 dojdą pola
- Rozdziel „definicję systemową" (lista umiejętności CP RED) od „danych postaci" (poziomy) — to fundament separacji rdzeń/system
- Arkusz projektuj od razu w układzie zbliżonym do karty CP RED (gracze ją znają), ale bez dopieszczania grafiki — szlif w etapie 27
